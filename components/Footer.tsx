...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-[#f7f3ec] border-t border-[#e6dfd2]">
      <div className="border-b border-[#e6dfd2] py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-x-10 gap-y-5 text-center sm:text-left">
          <nav aria-label={dict.footer.explore} className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {exploreLinks.map((l) => (
              <a key={l.id} href={l.href} className="text-xs font-medium uppercase tracking-wide text-[#6b645c] hover:text-[#14110f] transition-colors">
                {l.label}
              </a>
            ))}
          </nav>
          <span className="hidden sm:block w-px h-4 bg-[#e6dfd2]" />
          <nav aria-label={dict.footer.legal} className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            {legalLinks.map((l) => (
              <Link key={l.id} href={l.href} className="text-xs font-medium uppercase tracking-wide text-[#6b645c] hover:text-[#14110f] transition-colors no-underline">
                {l.label}
              </Link>
            ))}
          </nav>
      ...
      <div className="border-b border-[#e6dfd2] py-5 px-6">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8">
          <a
            ...
            className="flex items-center gap-2 text-[#6b645c] hover:text-[#14110f] transition-colors text-xs"
          >
          ...
          <span className="hidden sm:block text-[#d9d0c0] text-xs">·</span>
          <a
      ...
      <div className="py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-[#6b645c] text-sm">{dict.footer.copyright.replace('{year}', String(new Date().getFullYear()))}</p>
          <p className="text-[#8a8278] text-xs">{dict.footer.location}</p>
        </div>
      </div>
    </footer>
  );
